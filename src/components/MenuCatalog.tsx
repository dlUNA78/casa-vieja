...
import { Search, Plus, MessageSquare, Coffee, Check, Eye } from 'lucide-react';
...
export default function MenuCatalog({
  ...
  const [selectedCategory, setSelectedCategory] = useState<'Todos' | 'Desayunos' | 'Antojitos' | 'Bebidas' | 'Postres' | 'Especiales'>('Todos');
  ...
  const categories: ('Todos' | 'Desayunos' | 'Antojitos' | 'Bebidas' | 'Postres' | 'Especiales')[] = [
    'Todos', 'Desayunos', 'Antojitos', 'Bebidas', 'Postres', 'Especiales'
  ];
  ...
  const handleOpenAddNotes = (item: MenuItem, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleConfirmAddNotes = () => {
    if (selectedItemForNotes) {
      onAddToOrder(selectedItemForNotes, itemQuantity, customNotes);
  ...
  return (
    <div className="flex-1 p-6 md:p-8 overflow-y-auto scrollbar-hide flex flex-col space-y-6" id="menu-catalog-view">
              ...
              >
                ...
                <div className="p-4 flex-grow flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-stone-border/50">
                    <button
                      onClick={(e) => handleOpenAddNotes(item, e)}
                      ...
                      title="Agregar notas de preparación"
                    >
      ...
      {/* Add Item with customized notes Modal */}
      <AnimatePresence>
        {selectedItemForNotes && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
            <motion.div 
              ...
              className="bg-[#fdfcf0] rounded-2xl max-w-md w-full p-6 border border-stone-border shadow-xl space-y-6"
            >
              {/* Header */}
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <span className="text-[10px] bg-secondary/20 text-secondary font-sans font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                    {selectedItemForNotes.category}
                  </span>
                  <h3 className="text-xl font-serif text-on-surface font-bold mt-1">
                ...
                <div className="text-primary font-serif text-xl font-bold bg-primary-fixed text-primary px-3 py-1 rounded-xl">
                  ${selectedItemForNotes.price}
                </div>
              </div>

              {/* Quantity Selector */}
              <div className="space-y-2 border-t border-stone-border/60 pt-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-sm font-sans text-on-surface-variant">
                    Total: <span className="font-bold text-on-surface text-md">${(selectedItemForNotes.price * itemQuantity).toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-on-surface">Especificaciones o Notas</label>
                <textarea
                  value={customNotes}
                  onChange={(e) => setCustomNotes(e.target.value)}
                  placeholder="Ej. Sin cebolla, extra crema, salsa por separado, término medio, etc."
                  className="w-full p-3 rounded-xl bg-stone-card border border-stone-border focus:border-primary focus:ring-0 text-xs font-sans text-on-surface placeholder:text-on-surface-variant/40 h-24 resize-none transition-all"
                />
              </div>

              {/* Action buttons */}
              <div className="flex gap-3 border-t border-stone-border/60 pt-4">
                <button
